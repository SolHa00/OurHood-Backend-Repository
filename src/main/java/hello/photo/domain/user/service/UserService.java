    ...
    public DataResponse<UserLoginResponse> login(UserLoginRequest request, HttpServletResponse response) {
        try {
            ...
            if (!passwordEncoder.matches(request.getPassword(), userData.getPassword())) {
                return DataResponse.of(null, "비밀번호가 일치하지 않음");
            }

            Optional<User> userOptional = userRepository.findByEmail(request.getEmail());
            if(userOptional.isEmpty()) {
                return DataResponse.of(null, "해당 아이디가 존재하지 않음");
            }
            ...
            return DataResponse.of(userLoginResponse, Code.OK.getMessage());
        }
        catch (Exception e){
            return DataResponse.of(null, Code.NOT_FOUND.getMessage());
        }
    }

    public DataResponse<MyPageResponse> getMyPage(Long id) {
        ...
        return DataResponse.of(myPageResponse);
    }
